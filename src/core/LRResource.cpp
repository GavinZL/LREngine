...
LRResource::LRResource(ResourceType type)
    : mResourceID(GenerateResourceID())
    , mResourceType(type)
    , mRefCount(1)
    , mIsValid(false) {
}
...
uint32_t LRResource::GetRefCount() const {
    return mRefCount.load(std::memory_order_relaxed);
}

bool LRResource::IsValid() const {
    return mIsValid && GetNativeHandle().IsValid();
}

void LRResource::SetDebugName(const char* name) {
