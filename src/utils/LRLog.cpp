...
namespace {
    ...
    void OutputToConsole(const LogEntry& entry, const std::string& formattedTime) {
        ...
        const char* fileName = ExtractFileName(entry.file.c_str());
            ...
            if (s_color_enabled) {
                switch (entry.level) {
                    case LogLevel::Trace:   colorAttr = FOREGROUND_INTENSITY; break;
                    case LogLevel::Debug:   colorAttr = FOREGROUND_GREEN | FOREGROUND_BLUE | FOREGROUND_INTENSITY; break;
                    case LogLevel::Info:    colorAttr = FOREGROUND_GREEN | FOREGROUND_INTENSITY; break;
                    case LogLevel::Warning: colorAttr = FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_INTENSITY; break;
                    case LogLevel::Error:   colorAttr = FOREGROUND_RED | FOREGROUND_INTENSITY; break;
                    case LogLevel::Fatal:   colorAttr = BACKGROUND_RED | FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_BLUE | FOREGROUND_INTENSITY; break;
                    default: break;
                }
    ...
    void OutputToFile(const LogEntry& entry, const std::string& formattedTime) {
        ...
        s_log_file << "[" << formattedTime << "] [" << levelStr << "] [" 
                   << fileName << ":" << entry.line << "] " << entry.message << "\n";
...
void LRLog::Log(LogLevel level, const char* message) {
    LogEx(level, message, "", 0, "");
}

void LRLog::LogEx(LogLevel level, const char* message,
...
void LRLog::LogFormat(LogLevel level, const char* file, int32_t line, 
...
void LRLog::LogFormatV(LogLevel level, const char* file, int32_t line,
...
void LRLog::SetMinLevel(LogLevel level) {
    s_min_level = level;
}

LogLevel LRLog::GetMinLevel() {
    return s_min_level;
}

void LRLog::EnableConsoleOutput(bool enable) {
...
void LRLog::EnableColorOutput(bool enable) {
