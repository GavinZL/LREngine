...
RenderContextGLES::~RenderContextGLES() { Shutdown(); }
...
bool RenderContextGLES::Initialize(const RenderContextDescriptor& desc) {
    ...
    LR_LOG_INFO_F("[ContextGLES] Default VAO created: %u (not bound, each VBO has its own VAO)",
                  m_defaultVAO);
...
IShaderImpl* RenderContextGLES::CreateShaderImpl() { return new gles::ShaderGLES(); }

IShaderProgramImpl* RenderContextGLES::CreateShaderProgramImpl() {
    return new gles::ShaderProgramGLES();
}

ITextureImpl* RenderContextGLES::CreateTextureImpl() { return new gles::TextureGLES(); }

IFrameBufferImpl* RenderContextGLES::CreateFrameBufferImpl() { return new gles::FrameBufferGLES(); }

IPipelineStateImpl* RenderContextGLES::CreatePipelineStateImpl() {
    return new gles::PipelineStateGLES();
}

IFenceImpl* RenderContextGLES::CreateFenceImpl() { return new gles::FenceGLES(); }
...
void RenderContextGLES::EndRenderPass() {
    ...
    if (m_currentFrameBuffer != nullptr) {
        ...
        glViewport(0, 0, m_width, m_height);
        LR_LOG_TRACE_F("  Restored FBO: %d, Viewport: %ux%u", m_previousFrameBuffer, m_width,
                       m_height);
    }
...
void RenderContextGLES::DrawArrays(PrimitiveType primitiveType,
    ...
    LR_LOG_TRACE_F("OpenGL ES DrawArrays: type=%d, start=%u, count=%u", (int)primitiveType,
                   vertexStart, vertexCount);
    GLenum mode = gles::ToGLESPrimitiveType(primitiveType);
    glDrawArrays(mode, vertexStart, vertexCount);
}

void RenderContextGLES::DrawElements(PrimitiveType primitiveType,
                                     uint32_t indexCount,
                                     IndexType indexType,
                                     size_t indexOffset) {
    // 绘制前检查关键状态
    ...
    LR_LOG_INFO_F("[ContextGLES] Vertex Attrib Enabled: attr0=%d, attr1=%d", attr0Enabled,
                  attr1Enabled);
...
void RenderContextGLES::WaitIdle() { glFinish(); }

void RenderContextGLES::Flush() { glFlush(); }
