...
bool FrameBufferGLES::Create(const FrameBufferDescriptor& desc)
{
    ...
    if (colorCount > 0) {
        ...
        for (size_t i = 0; i < colorCount; ++i) {
            ...
            LR_LOG_DEBUG_F("OpenGL ES FBO color attachment %zu: tex=%u, format=0x%x", 
                           i, tex, internalFormat);
        }
    ...
    if (m_hasDepthStencil) {
        ...
        if (m_samples > 1) {
            ...
            glRenderbufferStorageMultisample(GL_RENDERBUFFER, m_samples, 
                                            depthInternalFormat, m_width, m_height);
#else
        ...
        LR_LOG_DEBUG_F("OpenGL ES FBO depth/stencil: RBO=%u, format=0x%x", 
                       m_depthStencilRBO, depthInternalFormat);
    }
    ...
    LR_LOG_DEBUG_F("OpenGL ES FBO created: ID=%u, size=%ux%u, colors=%zu, depth=%s",
                   m_fbo, m_width, m_height, m_colorTextures.size(),
                   m_hasDepthStencil ? "yes" : "no");
...
bool FrameBufferGLES::AttachColorTexture(ITextureImpl* texture, uint32_t index, uint32_t mipLevel)
{
    ...
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + index,
                           texGLES->GetTarget(), texGLES->GetTextureID(), mipLevel);
    glBindFramebuffer(GL_FRAMEBUFFER, 0);
