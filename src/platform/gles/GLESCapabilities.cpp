...
void GLESCapabilities::DetectExtensions() {
    ...
    hasTextureBorderClamp =
        HasExtension("GL_EXT_texture_border_clamp") || HasExtension("GL_OES_texture_border_clamp");
    hasTextureFloatLinear = HasExtension("GL_OES_texture_float_linear");

    // 渲染
    hasColorBufferFloat =
        HasExtension("GL_EXT_color_buffer_float") || HasExtension("GL_EXT_color_buffer_half_float");
...
void GLESCapabilities::LogCapabilities() const {
    ...
    LR_LOG_INFO_F("Anisotropic Filtering: %s (max: %.1f)", hasAnisotropicFiltering ? "Yes" : "No",
                  maxAnisotropy);
    ...
    LR_LOG_INFO_F("Max Uniform Blocks: VS=%d, FS=%d", maxVertexUniformBlocks,
