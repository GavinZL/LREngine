...
bool BufferGLES::Create(const BufferDescriptor& desc) {
    ...
    LR_LOG_DEBUG_F("OpenGL ES Buffer created: ID=%u, size=%zu, type=%d", m_bufferID, m_size,
                   (int)desc.type);
    return true;
}
...
void BufferGLES::Unmap() {
    ...
    if (m_mappedData != nullptr) {
        // 使用软件回退时，需要将数据写回
        if (m_mapAccess == MemoryAccess::WriteOnly || m_mapAccess == MemoryAccess::ReadWrite) {
...
void BufferGLES::Bind() {
    if (m_bufferID != 0) {
        ...
        if (m_target == GL_ELEMENT_ARRAY_BUFFER) {
            ...
            LR_LOG_INFO_F("[BufferGLES] Bind EBO: ID=%u, CurrentVAO=%d (EBO binding will be "
                          "recorded in VAO)",
                          m_bufferID, currentVAO);
            if (currentVAO == 0) {
                LR_LOG_WARNING(
                    "[BufferGLES] WARNING: EBO bound without VAO! EBO binding won't be recorded.");
            }
        } else {
...
void BufferGLES::Unbind() { glBindBuffer(m_target, 0); }
...
VertexBufferGLES::VertexBufferGLES() : m_vao(0), m_stride(0) {
    m_bufferType = BufferType::Vertex;
    m_target     = GL_ARRAY_BUFFER;
}

VertexBufferGLES::~VertexBufferGLES() { Destroy(); }
...
void VertexBufferGLES::Bind() {
    if (m_vao != 0) {
        ...
        if (currentVAO != (GLint)m_vao) {
            LR_LOG_ERROR_F("[BufferGLES] ERROR: VAO bind failed! Expected %u, got %d", m_vao,
                           currentVAO);
        }
    } else {
...
void VertexBufferGLES::Unbind() { glBindVertexArray(0); }
...
void VertexBufferGLES::SetVertexAttributes(const VertexAttribute* attributes, uint32_t count) {
    if (m_vao == 0 || attributes == nullptr || count == 0) {
        LR_LOG_ERROR_F("[BufferGLES] SetVertexAttributes: invalid params (VAO=%u, attrs=%p, "
                       "count=%u)",
    ...
    LR_LOG_INFO_F("[BufferGLES] SetVertexAttributes: VAO=%u, VBO=%u, count=%u, stride=%u", m_vao,
                  m_bufferID, count, m_stride);
    ...
    for (uint32_t i = 0; i < count; ++i) {
        ...
        LR_LOG_INFO_F("[BufferGLES]   Attr[%u]: location=%u, components=%d, type=0x%x, stride=%d, "
                      "offset=%u",
...
IndexBufferGLES::IndexBufferGLES() : m_indexType(IndexType::UInt32) {
    m_bufferType = BufferType::Index;
    m_target     = GL_ELEMENT_ARRAY_BUFFER;
}

IndexBufferGLES::~IndexBufferGLES() { Destroy(); }
...
UniformBufferGLES::UniformBufferGLES() : m_bindingSlot(0) {
    m_bufferType = BufferType::Uniform;
    m_target     = GL_UNIFORM_BUFFER;
}

UniformBufferGLES::~UniformBufferGLES() { Destroy(); }

bool UniformBufferGLES::Create(const BufferDescriptor& desc) { return BufferGLES::Create(desc); }
