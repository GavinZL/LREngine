...
void PipelineStateGL::ApplyBlendState()
{
    ...
    if (blend.enabled) {
        glEnable(GL_BLEND);
        glBlendFuncSeparate(
            ToGLBlendFactor(blend.srcColorFactor),
...
void PipelineStateGL::ApplyDepthStencilState()
{
    ...
    if (ds.stencilEnabled) {
        ...
        glStencilFuncSeparate(
            GL_FRONT,
            ToGLCompareFunc(ds.frontFace.compareFunc),
            ds.stencilRef,
            ds.stencilReadMask
        );
        glStencilOpSeparate(
            GL_FRONT,
            ToGLStencilOp(ds.frontFace.failOp),
        ...
        glStencilFuncSeparate(
            GL_BACK,
            ToGLCompareFunc(ds.backFace.compareFunc),
            ds.stencilRef,
            ds.stencilReadMask
        );
        glStencilOpSeparate(
            GL_BACK,
            ToGLStencilOp(ds.backFace.failOp),
...
void PipelineStateGL::ApplyRasterizerState()
{
    ...
    switch (raster.fillMode) {
        ...
        case FillMode::Point:     fillMode = GL_POINT; break;
    }
...
PrimitiveType PipelineStateGL::GetPrimitiveType() const
{
