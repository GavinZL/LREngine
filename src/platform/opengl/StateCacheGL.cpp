...
StateCacheGL::StateCacheGL() { Reset(); }
...
void StateCacheGL::SetViewport(int32_t x, int32_t y, int32_t width, int32_t height) {
    if (!m_valid || m_viewport.x != x || m_viewport.y != y || m_viewport.width != width ||
...
void StateCacheGL::SetScissor(int32_t x, int32_t y, int32_t width, int32_t height) {
    if (!m_valid || m_scissor.x != x || m_scissor.y != y || m_scissor.width != width ||
...
void StateCacheGL::SetColorMask(bool r, bool g, bool b, bool a) {
    if (!m_valid || m_colorMaskR != r || m_colorMaskG != g || m_colorMaskB != b || m_colorMaskA != a) {
...
void StateCacheGL::SetClearColor(float r, float g, float b, float a) {
    if (!m_valid || m_clearColor[0] != r || m_clearColor[1] != g || m_clearColor[2] != b ||
        m_clearColor[3] != a) {
