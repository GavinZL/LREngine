...
void TextureGL::SetSamplerParameters(const TextureDescriptor& desc)
{
    ...
    glTexParameteri(m_target, GL_TEXTURE_MAG_FILTER, 
                   ToGLFilterMode(desc.sampler.magFilter, false));
...
bool SamplerGL::Create(const SamplerDescriptor& desc)
{
    ...
    if (desc.compareMode) {
        ...
        glSamplerParameteri(m_samplerID, GL_TEXTURE_COMPARE_FUNC, ToGLCompareFunc(static_cast<CompareFunc>(desc.compareFuncValue)));
    }
...
void SamplerGL::Unbind(uint32_t slot)
{
